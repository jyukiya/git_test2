...
def add():
    ...
    c = conn.cursor()
    # DBにデータを追加する
    c.execute("insert into bbs values(null,?,?,0)", (user_id, comment))
    conn.commit()
    conn.close()
    return redirect('/bbs')
...
def delback():
    if 'user_id' in session :
        ...
        # fetchoneはタプル型
        user_info = c.fetchone()
        c.execute("select id,comment from bbs where userid = ? and DELETE_FLAG=1 order by id", (user_id,))
        comment_list = []
        for row in c.fetchall():
            comment_list.append({"id": row[0], "comment": row[1]})

        c.close()
        return render_template('delback.html' , user_info = user_info , comment_list = comment_list)
    else:
        return redirect("/login")

# 削除したテキストを復活！！！！
@app.route('/delback' ,methods=["POST"])
def delback_task():
    # クッキーから user_id を取得
    id = request.form.get("comment_id")
    id = int(id)
    conn = sqlite3.connect("service.db")
    c = conn.cursor()
    c.execute("update bbs set DELETE_FLAG = 0 where id = ?", (id,))
    conn.commit()
    c.close()
    return redirect("/delback")
